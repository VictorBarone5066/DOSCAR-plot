...
def GetDirection(ret):
    if(re.search("xStrain", ret)):
        return 'X'
    if(re.search("yStrain", ret)):
        return 'Y'    
    if(re.search("minEnergy", ret)):
        return 'Min'
...
def DOSGridGraphFull(pathToIn, pathToOut, outType, fileList):
    ...
    for f in fileList:
        ...
        if (f["type"] == 1):
            ...
            ax.plot([e - f["eFermi"] for e in f["energy"]], [-d for d in f["dos(dn)"]], color='k', linewidth=.5)   
